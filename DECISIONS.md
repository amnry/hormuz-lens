...
## 012. match_docs RPC over direct pgvector query from the JS client

Wrapped the pgvector similarity search in a Postgres function (`match_docs`) rather than issuing the `<=>` cosine operator directly from the Supabase JS client. Two reasons: (1) `@supabase/supabase-js` types express vector columns as `string`, so the JS layer must serialize the float array as `[x,y,...]` — the RPC encapsulates that wire format behind a typed interface. (2) The `<=>` operator lives in the `extensions` schema; without `set search_path = extensions, public, pg_catalog` in the function body, Postgres cannot resolve the operator and throws `operator does not exist`. Hiding that search-path requirement inside the RPC keeps it out of application code.